...
function flipColor(input){
    if(input.color === "blue"){ input.color = "red"}
    else{input.color = "blue"}
    return input
}
...
var getFullNames = function(input) {
    ...
    for(var i = 0;i < input.length; i++){
        var fullName = ''
            fullName = input[i].first + ' ' + input[i].last
...
dog.speak =function(string) {
    ...
    for(var i = 0;i < arr.length-1;i++){
        outputString += ruff(arr[i]) + ' '
    }
    outputString += ruff(arr[arr.length-1])
    return outputString
}
...
var wordCount = function(iterable,value){
    var stringArray = iterable.split(" ")
    var number = 0
        for (var pos = 0; pos < stringArray.length;pos = pos + 1){
            var element = stringArray[pos]
...
var getCounts = function(iterable){
    var stringArray = iterable.split(" ")
    var number = 0
    var newObj = {}
        for (var pos = 0; pos < stringArray.length;pos = pos + 1){
            var element = stringArray[pos]
            newObj[element] = wordCount(iterable,element)
        }
        return newObj
}
